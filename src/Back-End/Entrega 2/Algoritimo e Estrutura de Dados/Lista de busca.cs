...
class Dashboard
{
    ...
    public void EditarEventoOuProjeto()
    {
        ...
        if (escolha == "E")
        {
            ...
            if (evento == null) { Console.WriteLine("Evento não encontrado."); return; }

            Console.WriteLine($"\nTítulo: {evento.Titulo}");
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S") evento.Titulo = Console.ReadLine();

            Console.WriteLine($"Tipo: {evento.Tipo}");
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S") evento.Tipo = Console.ReadLine();

            Console.WriteLine($"Data: {evento.Data:dd/MM/yyyy}");
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S")
                evento.Data = DateTime.ParseExact(Console.ReadLine(), "dd/MM/yyyy", CultureInfo.InvariantCulture);

            Console.WriteLine($"Horário: {evento.Horario}");
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S") evento.Horario = Console.ReadLine();

            Console.WriteLine($"Local: {evento.Local}");
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S") evento.Local = Console.ReadLine();

            Console.WriteLine($"Status: {evento.Status}");
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S") evento.Status = Console.ReadLine();
        ...
        else if (escolha == "P")
        {
            ...
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S") projeto.Nome = Console.ReadLine();

            Console.WriteLine($"Status: {projeto.Status}");
            Console.Write("Editar? (S/N): "); if (Console.ReadLine().ToUpper() == "S") projeto.Status = Console.ReadLine();
    ...
    public void FiltrarEventosPorTipo()
    {
        ...
        if (filtrados.Count == 0) Console.WriteLine("Nenhum evento encontrado.");
        else
        {
        ...
        if (tipoPessoa == "palestrante")
        {
            foreach (var projeto in projetos)
            {
                foreach (var nome in projeto.ListaColaboradores)
                {
                    if (Normalizar(nome).Contains(nomeBusca))
                    {
                        Console.WriteLine($"{nome} - Projeto: {projeto.Nome} | Status: {projeto.Status} | (sem horário)");
                        encontrado = true;
                    }
                }
            }
